...
Pool.prototype._create = function() {
	...
	if (this._currentNumberOfConnections + this._currentNumberOfConnectionsEstablishing < this.max) {
		...
		log('Create new mariasql connection');
		...
		.on('end', function() {
			log('Done with all results, deposed this connection...');
		});
		// Return true.
		return true;
	}
				...
				connection.query(pending.query, pending.options)
					.on('result', function(result) {
						...
						result.on('data', function(row) {
								rows.push(row);
								log('Query: ', pending.query, 'returning row', rows.length);
							})
							.on('abort', function() {})
							.on('error', function(err) {
								// Send the error to the callback function.
								log('Query ', pending.query, 'error:', err);
								pending.fn(err, {
									query: result,
									rows: null,
									info: null
